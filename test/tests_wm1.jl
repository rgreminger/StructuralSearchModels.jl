...
@test estimates == [-0.011658551995099603, 1.4945882966369255,
 5.46636864421011, -0.8677224270695694]
...
@test m.cs == 3.2550068765896386e-9

# Welfare 
w = calculate_welfare(m, d, 100; rng, seed, method = "effective_values")
calculate_welfare(m, d, 100; rng, seed, method = "effective_values")
@test w[:average][:eff_value_choice] == 2.9740008937555813
@test w[:average][:welfare] == 2.9740008937555813
@test w[:conditional_on_purchase][:eff_value_choice] == 2.998533779392342
@test w[:conditional_on_purchase][:welfare] == 2.998533779392342


# Demand computation 
dem = [calculate_demand(m, d, i, j, 5; rng, seed) for i in 1:2, j in 1:2]
@test isequal(
    dem, [0.03690244166985573 1.875275451226644e-8; 
    0.036393388945450425 6.001942664422739e-7] )

# Revenue computation 
rev = calculate_revenues(m, d, 1, 5; rng, seed) 
@test rev[1] == -1.4356899872294822
@test rev[2][1:2] == [-0.361369698791823, -0.0023126112832036385]