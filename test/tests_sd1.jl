...
@test estimates == [-0.33380157449721964, 2.952996485252172,
    4.492705726089025, -0.1449647262037665, 2.517376324194926]
...
@test m.cd == 0.0003447560453978281
@test m.cs == 0.002004137179128191

# Welfare 
w = calculate_welfare(m, d, 100; rng, seed, method = "effective_values")
calculate_welfare(m, d, 100; rng, seed, method = "effective_values")
@test w[:average][:discovery_costs] == 0.008513750541099367
@test w[:average][:eff_value_choice] == 3.418247414691211
@test w[:conditional_on_purchase][:discovery_costs] == 0.009273746090065253
@test w[:conditional_on_search][:discovery_costs] == 0.00942015249067753
@test w[:conditional_on_purchase][:eff_value_choice] == 3.22686635230007
@test w[:conditional_on_search][:eff_value_choice] == 3.2510120783847696

# Demand computation 
dem = [calculate_demand(m, d, i, j, 5; rng, seed) for i in 1:2, j in 1:2]
@test isequal(
    dem, [0.6451646244745709 0.0051664017164279;
        0.6444150527878119 0.015511365724323661])

# Revenue computation 
rev = calculate_revenues(m, d, 1, 5; rng, seed) 
@test rev[1] == -0.9775653397690145
@test rev[2][1:2] == [-0.0645893438951959, -0.03469696420344893]
