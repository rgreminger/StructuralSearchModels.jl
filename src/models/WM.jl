...
#     zs_h = get(kwargs, :zs_h, zeros(T, length(d.positions[i])))
#     if zs_h[1] == 0
#         zsfun = get_functional_form(m.zsfun)
#         for h in eachindex(zs_h)
#             zs_h[h] = zsfun(m.ξ, m.ρ, positions[h])
#         end
#     end
...
#             prob_buy_u0 *= prob_not_buy(m, xβγ_j, xβκ_j, zs_h[j], u0_draw, dE, dV)
#         end
...
# function calculate_demand_product(m::WM{T}, d::DataSD, i, k, mapping_characteristics, n; kwargs...) where {T}
...
#     positions = @views d.positions[i]
#     zs_h = get(kwargs, :zs_h, zeros(T, length(positions)))
#     if zs_h[1] == 0
#         zsfun = get_functional_form(m.zsfun)
#         for h in eachindex(zs_h)
#             zs_h[h] = zsfun(m.ξ, m.ρ, positions[h])
...
#             prob_purchase_k *= cdf(dU0, wt_k - β[end])
#         end
...
#             prob_purchase_k *= prob_not_buy(m, xβγ_j, xβκ_j, zs_h[j], wt_k, dE, dV)
#         end
...
#     conditional_on_search = get(kwargs, :conditional_on_search, false)
#     if conditional_on_search
#         demand = demand / lik_no_searches(m, zd_h, m.ξ, m.β, m.γ, m.κ, m.dV, m.dU0, mapping_characteristics, d, i, n, true, rng, false, (xβγ, xβκ))
#     end
