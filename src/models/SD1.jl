...
function SDCore(m::SD1)
    SDCore(; β = m.β, Ξ = m.Ξ, ρ = m.ρ, ξ = m.ξ, ξρ = [0.0], cs = m.cs, cd = m.cd,
        dE = m.dE, dV = m.dV, dU0 = m.dU0, dW = Normal(0, 0), zdfun = m.zdfun,
        zsfun = "linear", unobserved_heterogeneity = m.unobserved_heterogeneity)
end

function calculate_welfare(
        model::SD1, data::DataSD, n_sim; method = "effective_values", kwargs...)
...
function generate_data(m::SD1, n_consumers, n_products; kwargs...)
...
function evaluate_fit(m::SD1, data::DataSD, n_sim; kwargs...)
...
function calculate_demand_product(m::SD1{T}, d::DataSD, i, k, n,
        β::Vector{T}, ξ::T, zd_h::Vector{T}, dE::Distribution,
...
@inline function prob_not_buy(m::Union{SD1, WM1}, xβ::T, ξ::T, u::T,
    ...
    P = cdf(dV, u - ξ - xβ) + cdf(Normal(), a / sqrt(1 + b^2)) -
        bvncdf(a / sqrt(1 + b^2), Y, -b / sqrt(1 + b^2))
    return P
end
